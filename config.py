...
class Config():
    ...
    @property
    def asset_path(self):
        """return the root of the asset directory path. The directory that host Task/Subtask/Version"""
    ...
    def asset_file_path(self):
        """
        return the template associated to the key "File" defined in the config file at config_path.
        The key/values correspondig to this template are defined in the config file located at "config_path"
        """
        return self.__templates["File"]

    @property
    def asset_file_path_dir(self):
        """
        return the folder of the asset file
        Exemple:
            self.asset_file_path_dir() # Output: "AssetName/..../v001/"
        """
        return self.__templates["FileDir"]
    ...
    def __init__(self, name, config_path="../Configurations/My_Projects.yml"):
        self.key_value = ["$Project", "$AssetPath","$FileDir", "$File", "$Workspace", "$Caches", "$Render", "$Flip", "$Textures",
                          "$AssetFileName"]  #
