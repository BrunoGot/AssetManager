...
class asset_view(QtWidgets.QWidget):
    """
   This part the sub window or widget used to represent the selected asset.
   todo: display all the dependencies of the asset on different level.... Dependences from out of the project
    and in the different tasks and subtasks aswell
     - Set different view that can be used to visualize an asset
     - possibility to clean some caches
     - read the Flip book and renders from VLC or an external video player
     - Access quickly to the final output of the asset
     - Possibility to archive an asset : pack all the dependency with it, ready to export,
            pack only selected dependancies, ready for storage
     - Possibility to render or add it to a render queue
   """
